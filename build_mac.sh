...
if [[ "$BUILD" == "true" ]]; then
    ...
    (echo $NAME && echo $ID && hostname && uptime) | git commit -F - -q --author='tracer-oscpu2021 <tracer@oscpu.org>' --no-verify --allow-empty 1>/dev/null 2>&1
    sync
fi
...
if [[ "$CHECK_WAVE" == "true" ]]; then
    cd $BUILD_PATH
    /Applications/gtkwave.app/Contents/Resources/bin/gtkwave $WAVE_FILE
    if [ $? -ne 0 ]; then
